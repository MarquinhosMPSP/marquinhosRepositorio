const siel = async (browser) => {
    ...
    let url = "http://ec2-18-231-116-58.sa-east-1.compute.amazonaws.com/siel/login.html";

    try {
        await page.goto(url, { waitUntil: 'networkidle2' });
        await page.focus('body > div.canvas > div.conteudo > div.mioloInterna.apps > form > table > tbody > tr:nth-child(1) > td:nth-child(2) > input[type=text]');

        await page.keyboard.type('email@email', { delay: 100 });
        await page.focus('body > div.canvas > div.conteudo > div.mioloInterna.apps > form > table > tbody > tr:nth-child(2) > td:nth-child(2) > input[type=password]');
        await page.keyboard.type('senha', { delay: 100 });

        await Promise.all([
            page.waitForNavigation(),
            page.click('body > div.canvas > div.conteudo > div.mioloInterna.apps > form > table > tbody > tr:nth-child(3) > td:nth-child(2) > input[type=submit]')
        ]);

        await Promise.all([
            page.focus('body > div.canvas > div.conteudo > div.mioloInterna.apps > form.formulario > fieldset:nth-child(1) > table > tbody > tr:nth-child(1) > td:nth-child(2) > input[type=text]'),
        ...
        await Promise.all([
            page.waitForNavigation(),
            page.click('body > div.canvas > div.conteudo > div.mioloInterna.apps > form.formulario > table > tbody > tr > td:nth-child(2) > input')
        ]);
        ...
        let data = await page.evaluate(() => {
            ...
            let nascimento = document.querySelector('body > div.canvas > div.conteudo > div.mioloInterna.apps > table > tbody > tr:nth-child(4) > td:nth-child(2)').innerText;
            ...
            let cdValidacao = document.querySelector('body > div.canvas > div.conteudo > div.mioloInterna.apps > table > tbody > tr:nth-child(13) > td:nth-child(2)').innerText;
