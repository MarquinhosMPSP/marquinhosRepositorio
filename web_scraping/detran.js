const detran = async (browser) => {
    ...
    await Promise.all([
        ...
        page.click('button[class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only botao"]')
    ]);
    ...
    await page.focus('input[class="ui-inputfield ui-inputmask ui-widget ui-state-default ui-corner-all"]');
    await page.keyboard.type('registro', {delay: 10});

    await page.focus('td[class="coluna4"] > input');
    await page.keyboard.type('PGU', {delay: 10});

    await page.focus('td[class="coluna6"] > input');
    await page.keyboard.type('CPF', {delay: 10});

    await page.focus('tr:nth-child(2)> td[class="coluna2"] > input');
    await page.keyboard.type('CNPJ', {delay: 10});

    const Href = await page.evaluate(() => {
    let href = document.querySelector('a[class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only botao"]').getAttribute("href");
      return href; 
    });
    
    let urlBase = "http://ec2-18-231-116-58.sa-east-1.compute.amazonaws.com/detran/";
    var pdf1 = urlBase + Href
    let relatorioLinhaDeVIdaNome = CPFformat + "_" + sysdateFormat + "_" + "Detran1.pdf";
    ...
    let relatorioLinhaDeVIda = pathPDF + CPFformat + "_" + sysdateFormat + "_" + "Detran1.pdf"; 
    ...
    await page.focus('input[class="ui-inputfield ui-inputtext ui-widget ui-state-default ui-corner-all"]');
    await page.keyboard.type('PLACA', {delay: 10});

    await page.focus('input[class="ui-inputfield ui-inputmask ui-widget ui-state-default ui-corner-all"]');
    await page.keyboard.type('CPF OU CNPJ', {delay: 10});
    ...
    let relatorioVeiculoNome = CPFformat + "_" + sysdateFormat + "_" + "Detran2.pdf";
    ...
    let relatorioVeiculo = pathPDF + CPFformat + "_" + sysdateFormat + "_" + "Detran2.pdf"; 
    ...
    await page.focus('input[class="ui-inputfield ui-inputtext ui-widget ui-state-default ui-corner-all"]');
    await page.keyboard.type('registroPGU', {delay: 10});

    await page.focus('input[class="ui-inputfield ui-inputmask ui-widget ui-state-default ui-corner-all"]');
    await page.keyboard.type('CPF', {delay: 10});

    const newPage2Promise = new Promise(x => browser.once('targetcreated', target => x(target.page())));
    ...
    let portalPathImg = pathImages + CPFformat + "_" + sysdateFormat + "_"  + 'detran.png';

    pageImagem.close();

    let portalPathPdf = [relatorioLinhaDeVIda, relatorioVeiculo]
